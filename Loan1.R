...
# Distribute of LoanAmount by Loan Status##
library(ggplot2)
...
#here I chose the second round of data imputation
sapply(tr, function(x) sum(is.na(x)))
...
logistic1 <- glm (as.factor(Loan_Status)~ Credit_History,data = trainnew, family = binomial)
summary(logistic1)

my_prediction_tr1 <- predict(logistic1, newdata = trainnew, type = "response")
table(trainnew$Loan_Status, my_prediction_tr1 > 0.5)

logistic_test1 <- glm (as.factor(Loan_Status)~ Credit_History,data = testnew, family = binomial)
...
logistic2 <- glm (as.factor(Loan_Status)~ Credit_History+Education+Self_Employed+Property_Area+LogLoanAmount+
...
logistic_test2 <- glm (as.factor(Loan_Status)~ Credit_History+Education+Self_Employed+Property_Area+LogLoanAmount+
